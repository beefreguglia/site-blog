...
export function Search() {
  ...
  function handleQueryChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    router.push(`blog?q=${encodeURI(newQuery)}`, undefined, {
      shallow: true,
      scroll: false,
    });
  }
  ...
  return (
    <form className="relative group w-full md:w-60" onSubmit={handleSearch}>
      ...
      {query && (
        <CircleX
          className="text-gray-300 absolute size-4 top-1/2 -translate-y-1/2 right-3 cursor-pointer hover:text-blue-300 transition-colors duration-200"
          onClick={resetSearch}
        />
      )}
    </form>
